...
RSpec.feature "Project Schedule", type: :feature do
  ...
  scenario "User visits the schedule page when month ends before week" do
    travel_to Date.new(2024, 7, 30) do
      end_of_week_task = create(:task, name: "End of Week Task", due_date: Date.current.end_of_week, project: project, user: user)
      ...
      expect(page).not_to have_content("This Month")

      within("section", text: "This Year") do
        expect(page).not_to have_content("End of Week Task")
      end
    end
  end
end

...
| `list_tasks` | List tasks with status, stars, due dates, and comment counts. Filter by status (`todo`, `done`, `all`) and/or `due` (`today`, `overdue`, `this_week`, `none`, or `YYYY-MM-DD`). |

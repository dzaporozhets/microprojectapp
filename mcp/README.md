...
| `list_tasks` | List tasks with status, stars, due dates, assignees, and comment counts. Filter by status (`todo`, `done`, `all`), `due` (`today`, `overdue`, `this_week`, `none`, or `YYYY-MM-DD`), and/or `assigned` (`me`, `unassigned`, or a numeric user ID). |
